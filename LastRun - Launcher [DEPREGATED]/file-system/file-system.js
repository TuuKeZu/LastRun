...
class FileSystem{
    ...
    Initialize = (application = app) => {
        return new Promise(async (resolve, reject) => {
            ...
            this.logPath = path.join(application.getPath('userData'), 'application-logs', 'log.txt');
            this.sessionDataPath = path.join(process.env.HOME, 'AppData', 'LocalLow', 'TuuKeZu', 'LastRun', 'session_data.txt');
    ...
    runVersion = async () => {
        return new Promise(async (resolve, reject) => {
            ...
            .then(() => {
                ...
                const process = execFile(EXEC_PATH, [], { cwd: __dirname }, (err, data) => {
                    if (err) return reject(err);
    ...
    SetLoginDetails = (details = {username: String, session_id: String}) => {
        return new Promise((resolve, reject) => {
            ...
            fs.writeFile(this.sessionDataPath, raw, (err) => {
                ...
                console.log("succesfully wrote session details!");
                return resolve(200);
            });
        });
    }
